...
NodeHandle SceneHandle::createNode(const QString& name, const QString &description, StoryNodeType storyType, bool load, const QUuid& uuid)
{
#ifdef QT_DEBUG
    Q_ASSERT(m_isValid);
#else
...
#endif
    return NodeHandle(m_scene->createNode(name, description, storyType, load, uuid));
}
